...
#+begin_src emacs-lisp
  (zb init-gtd
    ...
    ;; Per-project TODO.
    (defvar zy-project-todo-filenames
      '("TODO.org" "_zyxir_TODO.org")
      "Possible TODO filenames for projects.")
    (dolist (proj project--list)
      (let* ((proj-path (car proj))
             proj-todo
             proj-todo-tmp
             (proj-todo-filenames zy-project-todo-filenames))
        (while proj-todo-filenames
          (setq proj-todo-tmp (expand-file-name
                               (car proj-todo-filenames)
                               proj-path))
          (if (file-exists-p proj-todo-tmp)
              (setq proj-todo proj-todo-tmp
                    proj-todo-filenames nil)
            (setq proj-todo-filenames (cdr proj-todo-filenames))))
        (when proj-todo
          (push proj-todo org-agenda-files)))))
#+end_src
