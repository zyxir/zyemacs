...
#+begin_src emacs-lisp
  (zb init-proj
    ...
    (defun zy/refresh-projects ()
      "Refresh project list.

  This function:
  1. Runs `project-forget-zombie-projects'.
  2. Runs `project-remember-projects-under' on Zybox/projects."
      (interactive)
...
#+begin_src emacs-lisp
  (zb init-spellcheck
    :pkg 'wucuo
    ;; Use wucuo as spell checker, which shares interface with flyspell.
    (setq ispell-program-name "hunspell"
          ispell-local-dictionary "myhunspell"
          ispell-local-dictionary-alist '(("myhunspell" "[[:alpha:]]"
                                           "[^[:alpha:]]" "[']" nil
                                           ("-d" "en_US") nil utf-8)))
