...
*** ~zy!pick-font~ and ~zy!pick-main-font~
...
*** ~zy/fontlist-without-size~

#+begin_src emacs-lisp
  (defun zy/fontlist-without-size (fontlist)
    "Return a copy of FONTLIST, where size info is omitted."
    (let* ((new-fontlist (copy-sequence fontlist))
  ...
  (when (display-graphic-p)
    (zy!add-hook 'after-init-hook
      ...
      (defconst zy*fontlist-varpitch-preset
        `(,(zy!pick-main-font ("Libertinus Serif" 18)
                              ("Noto Serif" 18)
                              ("Times New Roman" 18)
                              ("serif" 18))
          ((han cjk-misc bopomofo)
        ...
        (dolist (level (number-sequence 1 8))
          (eval
           (append
            ...
            `(,(string-to-number
                (format "1.%d" (- 4 level)))))))
        (zy!apply-fontlist (zy/fontlist-without-size
                            zy/fontlist-default)
                           'org-code
                           'org-verbatim)
