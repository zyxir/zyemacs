...
#+begin_src emacs-lisp :tangle no
  '(主字體
    ...
    (字符集1 用於該字符集的字體)
    (字符集2 用於該字符集的字體)
    ((字符集3 字符集4 字符集5) 用於這些字符集的字體))
#+end_src

其中，若有任何字體的值爲 nil，或是在系統中沒有安裝，則會跳過它。

下面定義了函數​~zy/apply-fontlist~​，用來將某一個字體列表應用到某一個 face：

#+begin_src emacs-lisp
  (defun zy:apply-cflist-to-fontset (cflist fontset)
    "Apply charset-font list CFLIST to a FONTSET."
    (dolist (cfpair cflist)
      (let* ((charset (nth 0 cfpair))
	     (font (nth 1 cfpair))
	     (spec (font-spec :family font)))
	(when (find-font spec)
	  (if (listp charset)
	      (dolist (c charset)
		(set-fontset-font fontset c spec))
	    (set-fontset-font fontset charset spec))))))
  ...
  default for new frames only."
    (let ((face (if face face 'default))
	  (main-font (nth 0 fontlist))
	  (main-size (nth 1 fontlist))
	  (cflist (cdr (cdr fontlist))))
      ;; 設置主字體。
      (if (find-font (font-spec :family main-font))
	  (set-face-attribute
	   face
	   frame
	   :font (font-spec :family main-font
			    :size main-size))
	(set-face-attribute face frame :height (* 10 main-size)))
      ;; 獲取 FACE 的 fontset。如果是 'unspecified，則新建一個 fontset，
      ;; 並指定給它。
      (let ((fontset (face-attribute face :fontset frame)))
	...
	(zy:apply-cflist-to-fontset cflist fontset)
...
#+begin_src emacs-lisp
  (defvar zy/fontlist-default
    (if (boundp 'zy/fontlist-default)
        zy/fontlist-default
      '("Sarasa Mono Slab TC" 16
        ((kana han cjk-misc bopomofo) "Sarasa Mono Slab TC")))
    "Fontlist used for most occasions.")

  (defvar zy/fontlist-variable-pitch
    (if (boundp 'zy/fontlist-variable-pitch)
        zy/fontlist-variable-pitch
      '("Noto Serif" 18
        ((kana han cjk-misc bopomofo) "Noto Serif CJK TC")))
    "Fontlist used for the variable-pitch face.")

  (zy/apply-fontlist zy/fontlist-default 'default)
  (zy/apply-fontlist zy/fontlist-variable-pitch 'variable-pitch)
#+end_src
