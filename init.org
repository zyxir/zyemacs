...
- ~zy/NAME~ :: A public command designed to be used interactively, via =M-x= or a keybinding.
- ~zy-[-]NAME~ :: A non-interactive function.
- ~zy*NAME~ :: A constant.
- ~zy$NAME~ :: A variable.
- ~abc!~ :: A macro.
...
#+begin_src emacs-lisp
  (defconst zy*os-windows (eq system-type 'windows-nt)
    "If Emacs is run on Windows.")
  (defconst zy*os-linux (or (eq system-type 'gnu/linux)
                            (eq system-type 'linux))
    "If Emacs is run on Linux.

  Note that WSL is also Linux.")
  (defconst zy*os-wsl (equal
                       0
                       (shell-command
    ...
    (let* ((zybox-possible-locs-win64
           ...
           (guessed-zybox-path
            (cl-some
             ...
             (cond
              (zy*os-windows zybox-possible-locs-win64)
              (zy*os-linux zybox-possible-locs-linux)))))
...
#+begin_src emacs-lisp
  (setq
   ...
   fill-column 80
   frame-title-format (if zy*os-windows
                          '("" "windows-emacs" " [%b]")
                        '("" "emacs" " [%b]"))
...
#+begin_src emacs-lisp
  ...
  (defvar zy$use-solaire-mode
    (if (boundp 'zy$use-solaire-mode)
        zy$use-solaire-mode
      t)
    "Whether Emacs should install and enable 'solaire-mode'.")

  (when zy$use-solaire-mode
