...
#+begin_src elisp
  (defvar zy/project-path-list '()
    "A list of directories to search projects in.")
  (when zy/zybox-path
    (add-to-list 'zy/project-path-list
                 (expand-file-name "projects" zy/zybox-path)))

  (when *first-install*
    (with-eval-after-load "project"
      (dolist (dir zy/project-path-list)
...
字體列表的格式爲 ~'(默認字體 默認字號 變寬字體 變寬字號 中文字體)~​。

#+begin_src elisp
  (defvar zy/fonts-default
    (if (boundp 'zy/fonts-default)
        zy/fonts-default
      '("Sarasa Mono SC" 16 "Source Han Serif TC" 20 nil))
...
#+begin_src elisp
  ...
  If FRAME is omitted, use the current frame."
    (let ((font (nth 0 font-list))
          ...
          (vpfont (nth 2 font-list))
          (vpsize (nth 3 font-list))
          (cjkfont (nth 4 font-list)))
      (when font
        (set-face-attribute 'default frame :font
                            (font-spec :family font :size size)))
      (when vpfont
        (set-face-attribute 'variable-pitch frame :font
                            (font-spec :family vpfont :size vpsize)))
      (when cjkfont
